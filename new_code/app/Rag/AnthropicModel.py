...
"""
        self.prompt = PromptTemplate(
            ...
            input_variables=["context", "query", "internet_search_results"],
    ...
    def generate_answer_with_structure(self, context, query, schema: BaseModel, original_query=""):
        ...
        if original_query:
            print(f"🤖 ANTHROPIC THREAD: Using Gemini-powered Google Search for query: '{original_query}'")
    ...
    def stream_blocks(
        ...
        Called as:
            for event in llm_anthropic.stream_blocks(
        ...
        # if original_query:
        #     print(f"🤖 ANTHROPIC STREAM: Gemini-powered search for: '{original_query}'")
...
"""
    ...
    def generate_stream_answer_with_structure(self, context, query, schema, original_query=""):
        parser = PydanticOutputParser(pydantic_object=schema)
        if original_query:
            print(f"🤖 ANTHROPIC THREAD (STREAMING): Using Gemini-powered Google Search for query: '{original_query}'")
        search_results = self.search_node.search(original_query) if original_query else ""
        print('search results:', search_results)
        chain = self.get_prompt_with_parser(parser=parser) | self.get_stream_llm()
        result = chain.stream({"context": context, "query": query, "internet_search_results": search_results})
        for res in result:
            print(res.content, end="", flush=True)
        return result
