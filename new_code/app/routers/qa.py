...
# ─────────────────────────────────────────────────────────────
from app.access_validator.RootAuthorization import Authorization,AuthorizationContext
...
def ask(
    ...
    def event_generator(query=data.q.strip()):
        ...
        try:
            ...
            allowed = qa_service.allowed_thread_id(
                db=db,
            ...
            yield qa_service.json_line({
                "type": "stage",
                "value": "fetching relevant documents",
            })
            ...
            @traceable(
                ...
                project="core",
                metadata={
                    "description": "Fetch relevant documents for query"
                },
            ...
            docs_list = qa_service.rerank_docs(
                query=data.q,
                docs=docs_list,
                top_n=5,
            )
            ...
            top_rerank_score = (
                docs_list[0].metadata.get("rerank_score", 0.0)
                if docs_list
                else 0.0
            )
     
            is_grounded = top_rerank_score >= RELEVANCE_THRESHOLD
            ...
            yield qa_service.json_line({
                "type":"stage",
                "value":"checking for any sensitive info"
            })
            contain_sensitive_info=masking.analyze_docs(data.q,
                            docs_list,
                            masking_state)
            timer.mark("checking sensitive info")
            if contain_sensitive_info:
                yield qa_service.json_line({
                "type":"stage",
                "value":"found sensitive info"
                })


                yield qa_service.json_line({
                "type": "stage",
                "value": "masking sensitive information",
                 })
                ...
                masking_state,
                 ...
                 yield qa_service.json_line({
                                 "type": "stage",
                                 "value": "No sensitive information Present",
                                  })
                 masked_docs=docs_list
            timer.mark("masking")

            # ── Provider + memory config ───────────────────────────
            provider = qa_service.get_thread_provider(
                db,
                current_user,
                thread_id,
            )
            ...
            config = {
                "configurable": {
                    "thread_id": str(memory_thread_id)
                }
            }
            ...
            @traceable(
                ...
                project="core",
                metadata={
                    "description": "Load prior chat history for context"
                },
                tags=["threads", "users"],
            )
            # def load_chat_history():
            #     with qa_service.get_checkpointer() as checkpointer:
            #         chatbot = qa_service.builder(
            ...
            # docs_list is already sorted descending by rerank_score.
            # docs_list[0] is therefore the top match.
            ...
            for event in llm_instance.stream_blocks(
                ...
                if etype == "block":
                    ...
                    is_citation_required = event.get(
                        "required",
                        False,
                    )
                    ...
                    if is_citation_required:
                        ...
                        my_link = qa_service.normalize_citation_links(
                            raw_links
                        )
            ...
            # ── Citations ─────────────────────────────────────────
            if not is_grounded:
                # Retrieval itself says nothing relevant was found for
                # this query — deterministic, computed before the model
                # ever ran.
                my_link = []

            else:
                serialize_doc_list = qa_service.documents_to_dicts(
                    docs_list
                )
                ...
                citation_filenames = list(
                        dict.fromkeys(citation_filenames)
                    )
                ...
                # else: my_link stays [] — retrieval found relevant docs,
                # but the model itself said it didn't use them.
                ...
                chatbot.update_state(
                    config,
                    {
                        "messages": [
                            ...
                            AIMessage(
                                content=json.dumps(collected_blocks)
                            ),
                        ]
                    },
                )
            ...
            llm_response = extract_text_only_from_html(
                collected_blocks
            )
            ...
            chat_message.unanswer_query = (
                False if collected_blocks else True
            )
...
def edit_message(
    ...
    def event_generator(query=data.q.strip()):
        ...
        try:
            ...
            # --- Relevance gate (same logic as /ask/{thread_id}) ---
            # docs_list is sorted descending by rerank_score, so docs_list[0]
            # is the top match. If even the top match is below threshold,
            # we treat this turn as not grounded in the documents and
            # suppress citations regardless of what the LLM emits.
            RELEVANCE_THRESHOLD = 0.5
            top_rerank_score = (
                docs_list[0].metadata.get("rerank_score", 0.0) if docs_list else 0.0
            )
            is_grounded = top_rerank_score >= RELEVANCE_THRESHOLD
            ...
            # Gated: if retrieval wasn't grounded (top rerank score below
            # threshold), suppress citations entirely regardless of what
            # the LLM emitted.
            if is_grounded:
...
def ask_by_id(
    ...
    def event_generator(query=data.q.strip()):
        ...
        try:
            ...
            #    AND retrieval itself was relevant enough (is_grounded) ──
            if llm_emitted_citations and is_grounded:
                serialize_doc_list = qa_service.documents_to_dicts(docs_list)
                citation_filenames = []
                for d in serialize_doc_list:
                    meta = d.get("metadata") or {}
                ...
                # web / general knowledge / not-in-docs / not grounded → no citations
                my_link = []
                print("no citations — answer not from documents or retrieval not grounded")
